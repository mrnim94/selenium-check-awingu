...
// new 1 instance cho struct StructValidator
...
func (cv *StructValidator) RegisterValidate() {
	...
	// Đăng ký cho cái tags là pwd
	cv.Validator.RegisterValidation("pwd", func(fl validator.FieldLevel) bool {
		return len(fl.Field().String()) >= 8 // như thế nào là hợp lệ
	})
...
// Struct tructValidator đã overwrite Validate
